...
| 2 | Gaming parlor slot booking | `features/booking/...` + shared models `gaming_booking.dart`; providers/gaming_booking_provider.dart | → | `POST/GET /api/v1/gaming-bookings/*` etc. | → | `domains/gaming_booking/router.py` → `domains/gaming_booking/service.py` + `repository.py` + `parlor_router.py` + `gc_points.py` | → | `gaming_bookings`, `gaming_slots`, `parlour_offers`, `gc_points`, `gc_points_transactions` | → | `admin-microservice-complete/frontend/src/app/features/bookings/bookings-list.component.ts` + slots/ + offers/ (mocks currently); main real data at `backend/app/domains/admin/router.py` (/admin/gaming-bookings etc) |
