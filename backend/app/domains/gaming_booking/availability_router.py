"""Availability + v2 booking endpoints (Cashfree onboarding spec)."""
...
async def get_availability(
    ...
    slots = await AvailabilityService(db).compute_availability(parlor_id, date, station_type)
    return AvailabilityResponse(
        parlor_id=parlor_id, date=date, station_type=station_type, slots=slots
    )
...
async def create_booking_v2(
    ...
    booking = await svc.create_booking_v2(
        ...
        contact_phone=body.contact_phone or getattr(current_user, "phone", None) or getattr(current_user, "phone_number", None),
    ...
    if body.payment_mode == "online":
        ...
        # Schedule hold expiry (best-effort Celery)
        try:
...
async def booking_status(
    ...
    # Poll Cashfree if still pending
    if booking.booking_status == "payment_pending" and getattr(booking, "cf_order_id", None):
        try:
