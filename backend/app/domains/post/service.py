    ...
    async def create_post(
        ...
        *,
        ...
        post = Post(
            ...
            post_type=getattr(data, 'post_type', 'post'),
            title=getattr(data, 'title', None),
            description=getattr(data, 'description', None),
            visibility=getattr(data, 'visibility', 'public'),
            audience=getattr(data, 'audience', 'everyone'),
            game_types=getattr(data, 'game_types', []),
            tags=getattr(data, 'tags', []),
            hashtags=getattr(data, 'hashtags', []),
            mentions=getattr(data, 'mentions', []),
            location_parlor_id=getattr(data, 'location_parlor_id', None),
            allow_comments=getattr(data, 'allow_comments', True),
            allow_remix=getattr(data, 'allow_remix', True),
            allow_duet=getattr(data, 'allow_duet', True),
            hide_likes=getattr(data, 'hide_likes', False),
            is_ai_content=getattr(data, 'is_ai_content', False),
            is_paid_promo=getattr(data, 'is_paid_promo', False),
            is_for_kids=getattr(data, 'is_for_kids', False),
            duration_seconds=getattr(data, 'duration_seconds', None),
            thumbnail_asset_id=getattr(data, 'thumbnail_asset_id', None),
            video_asset_id=getattr(data, 'video_asset_id', None),
        )
