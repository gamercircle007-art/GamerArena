// sample-components/sidebar.component.ts
// COPY THIS PATTERN for all components.
// Standalone, signals, @if/@for, inject(), animations, Bootstrap 5 icons.
...
interface NavItem {
  ...
  permission?: string; superAdminOnly?: boolean;
  badge?: number;
}
...
const NAV_CONFIG: NavSection[] = [
  {
    ...
    items: [
      ...
      { label: 'Analytics',  icon: 'bi-bar-chart-line',     route: '/analytics', permission: 'view_platform_analytics' },
    ],
  },
  {
    section: 'MANAGEMENT',
    items: [
      ...
      { label: 'Tournaments',  icon: 'bi-trophy',               route: '/tournaments', permission: 'view_tournaments' },
      { label: 'Bookings',     icon: 'bi-ticket-perforated',    route: '/bookings',    permission: 'view_all_bookings' },
  ...
  },
  {
    ...
    items: [
      ...
      { label: 'Reels & Videos',icon: 'bi-camera-video',        route: '/posts/reels',     permission: 'view_posts' },
      { label: 'Comments',     icon: 'bi-chat-dots',            route: '/social/comments', permission: 'view_comments' },
      { label: 'Likes',        icon: 'bi-heart',                route: '/social/likes',    permission: 'view_likes' },
      { label: 'Community',    icon: 'bi-globe2',               route: '/community',       permission: 'view_community' },
    ],
  },
  {
    section: 'SOCIAL',
  ...
  },
  {
    section: 'SYSTEM',
    items: [
      { label: 'Broadcast',    icon: 'bi-megaphone',            route: '/notifications', permission: 'send_broadcast' },
...
@Component({
  ...
  template: `
    <nav class="sidebar" [@sidebarState]="collapsed() ? 'collapsed' : 'expanded'">
      ...
      @if (!collapsed()) {
        <div class="sidebar-footer">
          <div class="user-info">
            <div class="user-avatar">{{ authService.currentUser()?.name?.[0]?.toUpperCase() }}</div>
            <div class="user-details">
              <span class="user-name">{{ authService.currentUser()?.name }}</span>
              <span class="user-role badge bg-primary">{{ authService.currentUser()?.role }}</span>
            </div>
          </div>
  ...
  styles: [`
    ...
    .user-name { color: #fff; font-size: 12px; font-weight: 600; display: block; overflow: hidden; text-overflow: ellipsis; }
    .btn-logout {
